...
export class CreateAllModels1707119462148 implements MigrationInterface {
    name = 'CreateAllModels1707119462148'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`book\` (\`id\` int NOT NULL AUTO_INCREMENT, \`title\` varchar(255) NOT NULL, \`summary\` varchar(255) NOT NULL, \`isbn\` datetime NOT NULL, \`authorId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`book_instance\` (\`id\` int NOT NULL AUTO_INCREMENT, \`imprint\` varchar(255) NOT NULL, \`dueBack\` datetime NOT NULL, \`status\` enum ('Available', 'Maintenance', 'Loaned', 'Reserved') NOT NULL DEFAULT 'Maintenance', \`bookId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
